...
const AddSongToPlaylist = ({ onClose, onSubmit }) => {
  ...
  useEffect(() => {
    const fetchPlaylists = async () => {
      try {
        const id = jwtDecode(localStorage.getItem("jwt")).sub;
        const response = await axiosInstance.get(`playlists/${id}/owned`);
  ...
  const handleSubmit = () => {
    ...
    setSuccessMessage("Song added successfully to playlist");
  };
  ...
  return (
    <>
      <DialogBackdrop
        ...
        onClick={onClose}
      />
      <PopupContainer id="addSongToPlaylistPopUp">
        <Button52 onClick={onClose} aria-label="Close" id="closeButton">
          ❌
        </Button52>
        {!successMessage ? (
          <>
            <h2>Add the song to a playlist</h2>
